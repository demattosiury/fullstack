...
from app.models.user import UserCreate, UserRead, UserWithToken
...
@router.post("/registrar", response_model=UserWithToken)
...
async def login(
    ...
    try:
        ...
        logger.error(
            f"Erro na requisição para autenticar usuário ({form_data.username}): {e}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
